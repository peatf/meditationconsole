...
const sketch = (p) => {
  ...
  class Wave {
    constructor() {
      this.radius = 0;
      this.speed = p.map(energyLevel, 0, 1, 1, 5); // Map speed to energyLevel
      this.segments = 120;
    ...
    display() {
      ...
      for (let i = 0; i < this.segments; i++) {
        ...
        let colorOffset =
          p.noise(this.radius * 0.02, i * 0.05, p.frameCount * 0.01) * 50 - 25;
  ...
  const handleMove = (currentY) => {
    if (isDragging) {
      ...
      energyLevel = p.constrain(energyLevel, 0, 1); // Constrain energyLevel
      lastY = currentY;
    }
  };
  ...
  p.windowResized = () => {
    ...
    const h = containerElement?.offsetHeight || window.innerHeight;
    p.resizeCanvas(w, h);
    noiseGraphics = p.createGraphics(w, h); // Recreate noiseGraphics
    generateNoiseTexture();
  };
};
